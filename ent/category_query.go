...
func (cq *CategoryQuery) QueryEvents() *EventQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, category.EventsTable, category.EventsColumn),
		)
...
func (cq *CategoryQuery) WithEvents(opts ...func(*EventQuery)) *CategoryQuery {
	query := &EventQuery{config: cq.config}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context) ([]*Category, error) {
	...
	if query := cq.withEvents; query != nil {
		...
		query.Where(predicate.Event(func(s *sql.Selector) {
			s.Where(sql.InValues(category.EventsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.category_events
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "category_events" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "category_events" returned %v for node %v`, *fk, n.ID)
			}
