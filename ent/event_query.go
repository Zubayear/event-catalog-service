...
func (eq *EventQuery) QueryOwner() *CategoryQuery {
...
func (eq *EventQuery) WithOwner(opts ...func(*CategoryQuery)) *EventQuery {
	query := &CategoryQuery{config: eq.config}
...
func (eq *EventQuery) sqlAll(ctx context.Context) ([]*Event, error) {
	...
	if query := eq.withOwner; query != nil {
		...
		for i := range nodes {
			if nodes[i].category_events == nil {
				continue
			}
			fk := *nodes[i].category_events
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "category_events" returned %v`, n.ID)
			}
			for i := range nodes {
